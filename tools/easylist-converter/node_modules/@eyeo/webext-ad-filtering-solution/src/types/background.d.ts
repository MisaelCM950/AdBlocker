...
export interface Facts {
  ...
  countryCode?: string;
  /**
   * Whether the user has an active freemium trial.
   */
  isTrial?: boolean;
  /**
   * Number of days remaining in freemium trial period, or zero if not on trial.
   */
  ...
  /**
   * The timestamp where the user started the reverse trial for zero delay snippets.
   */
  reverseTrialStartDate?: number;
  /**
   * The number of active custom (user-added) filters.
   */
  customFiltersCount?: number;
  /**
   * The number of active dynamic declarative net request rules (MV3 only).
   */
  dynamicRuleCount?: number;
  /**
   * The number of enabled static declarative net request rulesets (MV3 only).
   */
  staticRuleCount?: number;
  /**
   * 12-character SHA-256 hash prefixes of active filter list subscription URLs.
   */
  filterLists?: string[];
  /**
   * The timestamp when the blockthrough tag was last seen in milliseconds.
   */
  lastBlockthroughTagDate?: number;
  /**
   * The timestamp when a bait request was last blocked in milliseconds.
   */
  lastBaitBlockedDate?: number;
  /**
   * Whether an ABP or AB co-install has been detected.
   */
  partnerExtensionCoinstalled?: boolean;
}
...
export interface UserProfile {
  ...
  /**
   * User subscription information including trial status
   */
...
export interface AddonInfo {
  ...
  remoteConfig?: {
    ...
    */
    ...
    /**
     * Host-specific rules index mapping subscription URLs to static rule
     * counts, used to compute staticRuleCount in MV3.
     */
    rulesIndex?: Array<{ url: string; rule_count?: number | null }>;
  };
  ...
  /**
   * Default value for Lightning Snippets (zero-delay filter caching).
   * Defaults to `false` when omitted. Acts as the lowest-priority value
   * in the `lightningSnippets` namespace precedence chain — runtime
   * calls to `ewe.lightningSnippets.enable()` / `disable()` override
   * it, and premium users are enabled when no runtime opinion has been
   * set.
   */
  lightningSnippets?: boolean;
}
...
declare module "@eyeo/webext-ad-filtering-solution" {
  ...
  /**
   * Signals that post-start setup is complete, unblocking Safari polyfill
   * operations that are gated during startup. No-op on non-Safari builds.
   */
  const signalStartupComplete: () => void;
  ...
  namespace filters {
    ...
    /**
     * Validates a filter.
     * @param text - Filter text to be validated.
     * @returns `null` if the filter is valid, otherwise a `FilterError` describing why it is not.
     */
    const validate: (text: string) => FilterError | null;
  }
  ...
  namespace account {
     ...
     */
    const isTrial: () => Promise<boolean>;

    /**
     * Retrieves the user's preferences
     */
    const getPreferences: () => Promise<Record<string, boolean>>;

    /**
     * Retrieves the user's trial days left
     */
    const getTrialDaysLeft: () => Promise<number>;
  ...
  namespace remoteConfig {
     ...
     *
     * The result is the concatenation of the top-level `ipmCommands` from
     * the remote config and any `ipmCommands` carried by experiment
     * variants the current client is assigned to.
     *
    ...
    /**
     * Re-evaluates cohorts and experiments using the current facts, without
     * re-downloading the remote config.
     */
    const updateFacts: () => Promise<void>;
  }
  ...
  namespace experiments {
    ...
    /**
     * Clears a single feature flag override, restoring it to the
     * remote-configured value.
     */
    const clearFlagOverride: (flagId: string) => Promise<void>;
    /**
    ...
    const getFlag: (
      flag: string
    ) => Promise<boolean | string | number | string[] | Record<string, unknown> | null>;
    /**
  ...
  namespace lightningSnippets {
    /**
     * Enables Lightning Snippets (zero-delay filter caching).
     * Overrides premium status and the default. Persists across SDK
     * restarts until `clear()` is called.
     */
    const enable: () => Promise<void>;
    /**
     * Disables Lightning Snippets. Overrides premium status and the
     * default — a premium user disabled here stays disabled until
     * `clear()` is called.
     */
    const disable: () => Promise<void>;
    /**
     * Removes the caller opinion, letting resolution fall through to
     * premium status, then the default.
     */
    const clear: () => Promise<void>;
    /**
     * Resolves whether Lightning Snippets is enabled. Precedence
     * (highest to lowest): caller opinion (`enable`/`disable`),
     * premium status, default (`addonInfo.lightningSnippets`).
     */
  ...
  namespace telemetry {
    ...
    /**
     * Returns the rotating client ID used for privacy-preserving telemetry.
     * The ID rotates every 7 days by default.
     * @returns {Promise<string>} Promise that resolves with the rotating
     * client ID.
     */
    export function getRotatingClientId(): Promise<string>;

    /**
     * Emitted when the rotating client ID has changed.
     */
    const onRotatingClientIdChanged: EventDispatcher<
        (rotatingClientId: string) => unknown
    >;
