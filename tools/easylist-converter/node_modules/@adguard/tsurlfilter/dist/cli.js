...
import { RawRuleConverter, RuleCategory, NetworkRuleType, RuleParser } from '@adguard/agtree';
import * as z from 'zod';
import { z as z$1, ZodError } from 'zod';
import { fromZodError } from 'zod-validation-error';
import { RuleConverter } from '@adguard/agtree/converter';
import { RuleGenerator } from '@adguard/agtree/generator';
import { DomainListParser, defaultParserOptions, NetworkRuleParser } from '@adguard/agtree/parser';
 ...
 */
const nonNegativeIntegerSchema = z$1.union([z$1.string(), z$1.number()])
    .pipe(z$1.coerce.number())
...
z$1.object({
     ...
     */
    conversions: z$1.record(nonNegativeIntegerSchema, z$1.number()),
});
/**
...
const URLTransformValidator = z$1.strictObject({
    ...
    scheme: z$1.string().optional(),
    username: z$1.string().optional(),
});
/**
 * https://developer.chrome.com/docs/extensions/reference/declarativeNetRequest/#type-Redirect
 */
const RedirectValidator = z$1.strictObject({
    extensionPath: z$1.string().optional(),
    regexSubstitution: z$1.string().optional(),
 ...
 */
const ModifyHeaderInfoValidator = z$1.strictObject({
    header: z$1.string(),
    operation: z$1.nativeEnum(HeaderOperation),
 ...
 */
const RuleActionValidator = z$1.strictObject({
    ...
    type: z$1.nativeEnum(RuleActionType),
});
/**
 ...
 * Chrome 128+
 */
const HeaderInfoValidator = z$1.strictObject({
    excludedValues: z$1.string().array().optional(),
    header: z$1.string(),
    values: z$1.string().array().optional(),
});
/**
 * https://developer.chrome.com/docs/extensions/reference/declarativeNetRequest/#type-RuleCondition
 */
const RuleConditionValidator = z$1.strictObject({
    domainType: z$1.nativeEnum(DomainType).optional(),
    excludedInitiatorDomains: z$1.string().array().optional(),
    excludedRequestDomains: z$1.string().array().optional(),
    excludedRequestMethods: z$1.nativeEnum(RequestMethod).array().optional(),
    excludedResourceTypes: z$1.nativeEnum(ResourceType).array().optional(),
    excludedTabIds: z$1.number().array().optional(),
    initiatorDomains: z$1.string().array().optional(),
    isUrlFilterCaseSensitive: z$1.boolean().optional(),
    regexFilter: z$1.string().optional(),
    requestDomains: z$1.string().array().optional(),
    requestMethods: z$1.nativeEnum(RequestMethod).array().optional(),
    /**
     ...
     */
    resourceTypes: z$1.nativeEnum(ResourceType).array().optional(),
    tabIds: z$1.number().array().optional(),
    urlFilter: z$1.string().optional(),
});
/**
 * https://developer.chrome.com/docs/extensions/reference/declarativeNetRequest/#type-Rule
 */
const DeclarativeRuleValidator = z$1.strictObject({
...
const serializedRuleSetLazyDataValidator = z$1.strictObject({
    sourceMapRaw: z$1.string(),
    filterIds: z$1.number().array(),
});
const serializedRuleSetDataValidator = z$1.strictObject({
    regexpRulesCount: z$1.number(),
    unsafeRulesCount: z$1.number(),
    rulesCount: z$1.number(),
    ruleSetHashMapRaw: z$1.string(),
    badFilterRulesRaw: z$1.string().array(),
 ...
 * @returns Promise that resolves to true if the regex is supported, and rejects with an error otherwise.
 */
const regexValidatorNode = async (regexFilter) => {
 ...
 */
const convertFilters = async (filtersAndMetadataDir, resourcesDir, destRulesetsDir, options = {}) => {
    const { debug = CONVERT_FILTER_DEFAULT_OPTIONS.debug, prettifyJson = CONVERT_FILTER_DEFAULT_OPTIONS.prettifyJson, additionalProperties, } = options;
    ...
    const filters = files
        ...
        return new Filter(filterId, { getContent: async () => new FilterList(data) }, 
        // we consider that all preinstalled filters are trusted
        true);
    })
        .filter((filter) => filter !== null);
    const convertedRulesets = [];
    let errors = [];
    let limitations = [];
    ...
    const rawMetadata = await fs.promises.readFile(path.join(filtersAndMetadataDir, LOCAL_METADATA_FILE_NAME), { encoding: 'utf-8' });
...
class Extractor {
     ...
     */
    static async extractMetadata(jsonFilePath, outputPath) {
        const rawJson = await fs$1.readFile(jsonFilePath, 'utf8');
        ...
        await fs$1.writeFile(path.join(outputPath, LOCAL_METADATA_FILE_NAME), JSON.stringify(metadata, null, 2));
    }
    /**
     ...
     */
    static async extract(rulesetsPath, outputPath) {
        ensureDirSync(outputPath);
        const rulesetsPaths = await findFiles(rulesetsPath, (filePath) => filePath.endsWith(RULESET_FILE_EXT));
        for (const rulesetPath of rulesetsPaths) {
            ...
            try {
                ...
                const data = await fs$1.readFile(jsonFilePath, 'utf8');
                ...
                await fs$1.writeFile(outputFilePath, originalFilterListText);
...
/* eslint-disable no-console */
const DEFAULT_DEST_RULE_SETS_DIR = './build/rulesets';
const CLI_NAME = 'tsurlfilter';
const parseBool = (v) => /^true|1|yes|on$/i.test(v);
/**
...
async function main() {
    ...
    program
        ...
        .action(async (rulesetsPath, outputPath) => {
        try {
...
export { DEFAULT_DEST_RULE_SETS_DIR, convertFilters, generateMD5Hash };
